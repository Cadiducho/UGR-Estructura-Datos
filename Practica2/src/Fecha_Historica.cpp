...
//Operador >>
  istream& operator>> (istream& is, Fecha_Historica& e) {
    ...
    is >> aux.anio >> separador;

    cout << "Year " << aux.anio << endl;
    //bucle para obtener todas las string
    int i = 0;
    bool salir = false;
    do {
      i++;
      getline(is, acontecimiento, separador);
      cout << "pelicula " << i << ": " << acontecimiento << endl;

      aux.aniadirevento(acontecimiento);
      if (is.peek() == '\n') salir = true;
    } while (is && !salir);
