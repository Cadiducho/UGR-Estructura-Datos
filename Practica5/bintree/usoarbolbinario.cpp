...
template <class T>
int Altura(const bintree<T>& a, const typename bintree<T>::node n){
  int iz,de;
  if (n.null()) {
    return -1;
} else {
...
template <class T>
void Esquema(const bintree<T>& a, const typename bintree<T>::node n, string& pre){
  int i;
...
template <class T>
int NumHojas(const bintree<T>& a, const typename bintree<T>::node n) {
    for (typename bintree<T>::const_level_iterator i = a.begin_level(); i!=a.end_level(); ++i) {
        if (esHoja(a, *i)) hojas++;
    }
...
cout << "La altura es: " << Altura(Arb, Arb.root()) << endl;
cout << "El numero de hojas es: " << NumHojas(Arb) << endl;
cout << endl;

cout << "Tiene el esquema:" << endl;
string pre="";
